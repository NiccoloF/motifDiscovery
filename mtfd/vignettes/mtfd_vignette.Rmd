...
```{r motif_position_pointiwise_error,eval = FALSE}
mot_len <- 100 
...
```{r motif_position_coeff_error,eval = FALSE}
mot_len <- 100
# struct specifying the motif ID,the number of curve and the relative knot position
motif_str <- rbind.data.frame(c(1, 1, 20),
                              c(1, 1, 2), 
...
```{r random_position_pointiwise_error,eval = FALSE}
mot_len <- 100
...
```{r random_position_coeff_error,eval = FALSE}
mot_len <- 100
